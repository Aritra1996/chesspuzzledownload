...
cursor.execute("CREATE INDEX IF NOT EXISTS idx_pt_theme_rating ON puzzle_themes (theme, rating)")
cursor.execute("""
...
cursor.execute("CREATE INDEX IF NOT EXISTS idx_po_opening_rating ON puzzle_openings (opening, rating)")
conn.commit()
...
UPSERT_SQL = """
    ...
    VALUES (?, ?, ?, ?, ?, ?)
"""
THEME_SQL   = "INSERT OR IGNORE INTO puzzle_themes (puzzle_id, theme, rating) VALUES (?, ?, ?)"
OPENING_SQL = "INSERT OR IGNORE INTO puzzle_openings (puzzle_id, opening, rating) VALUES (?, ?, ?)"
...
for row in ds:
    ...
    if len(batch) == 500:
        ...
        theme_rows   = [(r[0], t, r[3]) for r in batch if r[4] for t in r[4].split()]
        opening_rows = [(r[0], o, r[3]) for r in batch if r[5] for o in r[5].split()]
        if theme_rows:
